  <section id="about" class="my-16">
    <div class="max-w-7xl mx-auto mb-10 flex justify-between gap-15 items-center">
    <div class="flex flex-col items-start">
      <h1 class="text-4xl font-bold py-8 mt-10 text-gray-900">Always have a Plan Z</h1>
      <h2 class="mt-8 text-xl text-gray-500">
        Have a dream to pursue is the most fulfilling aspect of life I can imagine. 
        Hurdles with arise. 
        Challenges will be faced.
        Backup plans will be made.
        Nonetheless, I will continue to strive to solve problems and achieve my goals in my own way by being a
        <strong> Software Engineer.</strong>
        <!-- I can make this more personal and adding more images of myself -> small tho -->
         <!-- Like a sticky notes board -> I like to add some mess into it. Not too ordered -->
      </h2>
      <p class="mt-6 text-base text-gray-500">
        If you share the same passion for problem-solving, innovation, and outlook.
      </p>
    </div>
    <img src="/jiajason.png" alt="JiaJason's Pic" class="md:inline hidden right-0 rounded-full max-h-115 shadow-2xl shadow-indigo-500/30">
     </div>
  </section>
